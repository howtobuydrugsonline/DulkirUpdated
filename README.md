...
```bash
git clone https://github.com/howtobuydrugsonline/DulkirUpdated.git
cd DulkirUpdated
./gradlew build
```
...
The mod lands in `build/libs/` as `DulkirMod-Fabric-1.1.4-26.1.2.jar`. Ignore the
`-sources.jar` — that one is for developers, not for playing.

The [`jarvis-fabric/`](jarvis-fabric) folder holds a port of the Jarvis HUD
editor library, which had no Minecraft 26.x release of its own. It is built
automatically as part of the command above (a Gradle composite build) and
**bundled into the output jar**, along with Cloth Config and Orbit — so the
single jar is the only file you install.
...
1. Install [Fabric Loader](https://fabricmc.net/use/installer) 0.19.3+ for
   Minecraft **26.1.2**.
2. Put these in your `.minecraft/mods` folder:
   - the `DulkirMod-Fabric-*.jar` from `build/libs/` (or from Releases)
   - [Fabric API](https://modrinth.com/mod/fabric-api) `0.155.2+26.1.2`
   - [Fabric Language Kotlin](https://modrinth.com/mod/fabric-language-kotlin) `1.13.13+`
3. Make sure the profile runs on **Java 25+**, or the mod will not load. That is
   the most common reason it fails to show up.
...
This launches without authentication — enough to reach the main menu, not enough
to join a server. To sign in, configure `~/.devauth/config.toml` and run
